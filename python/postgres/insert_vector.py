...
# Prepare new entries with timestamp
now = datetime.now(timezone.utc) 
new_entries = [(text, model.encode(text).tolist(), now) for text in texts if text not in existing_texts]

# Insert only missing texts
if new_entries:
    execute_values(cur, "INSERT INTO embeddings (text, embedding, created_at) VALUES %s", new_entries)
    conn.commit()
